...
const CustomerOrders = () => {
  ...
  const token = JSON.parse(localStorage.getItem('customer'))?.customer_token;
  ...
  useEffect(() => {
    ...
    const fetchOrderAndServices = async () => {
      try {
        const res = await orderService.getOrderByCustomerId(id, token);
        if (!res || res.length === 0) {
  ...
  if (!order) return <p style={{height: '30vh',fontSize:'20px'}} className="text-center mt-5">No Order Yet!!, Spots are filling up quickly! Reserve your order today.</p>;
